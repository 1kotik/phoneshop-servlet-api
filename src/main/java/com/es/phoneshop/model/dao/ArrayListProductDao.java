...
public class ArrayListProductDao implements ProductDao {
    ...
    public synchronized List<Product> findProducts(String query, SortCriteria sortCriteria, SortOrder sortOrder) {
        ProductQueryUtils.setQuery(query);
        return new ArrayList<>(products.stream()
                .filter(product -> ProductQueryUtils.getQueryMatchNumber(product.getDescription()) != 0)
                .sorted(getProductComparator(sortCriteria, sortOrder))
                .map(Product::new)
                .toList());
    }
    ...
    private Comparator<Product> getProductComparator(SortCriteria sortCriteria, SortOrder sortOrder) {
        Comparator<Product> comparator = new ProductDescriptionComparator();
        if (sortCriteria.equals(SortCriteria.DESCRIPTION)) {
            comparator = Comparator.comparing(Product::getDescription);
        }
        if (sortCriteria.equals(SortCriteria.PRICE)) {
